#!/usr/bin/env python3
"""Send a status e-mail for the lotto runner.

Config comes from environment variables (set via EnvironmentFile= in the
systemd unit, e.g. /etc/lotto.env):
...
def build_message(
    subject: str, body_file: Path, attachments: list[Path]
) -> EmailMessage:
